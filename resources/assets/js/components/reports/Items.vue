<template>
    <div>
		...
		<button class = "btn btn-info" disabled>Total Profit. : {{ total_profit }}</button>
	      <table class="table table-bordered table-striped table-dark">
	          ...
	          <tr>
				...
				<th v-on:click="sort('qty_sold',6)">Qty Sold &nbsp;<span v-show="showAsc[6]" class="glyphicon glyphicon-chevron-up"></span> <span v-show="showDesc[6]" class="glyphicon glyphicon-chevron-down"></span></th>
	            <th v-on:click="sort('total_sold',7)">Total Sold &nbsp;<span v-show="showAsc[7]" class="glyphicon glyphicon-chevron-up"></span> <span v-show="showDesc[7]" class="glyphicon glyphicon-chevron-down"></span></th>
				<th v-on:click="sort('profit',8)">Profit &nbsp;<span v-show="showAsc[8]" class="glyphicon glyphicon-chevron-up"></span> <span v-show="showDesc[8]" class="glyphicon glyphicon-chevron-down"></span></th>
	          </tr>
	          </thead>
	          <tbody>
...
<script>
	export default {
        ...
        mounted() {
            var app = this;
            axios.get('multicom-pos/public/api/v1/items/func/report?page='+ app.pagination.current_page)
                .then(function (resp) {
            ...
            axios.get('multicom-pos/public/api/v1/sales/func/profit')
            	.then(function (resp) {
                    app.total_profit = resp.data;
                })
                .catch(function (resp) {
                    console.log(resp);
        ...
        methods: {
			...
			refresh(){
				var app = this;
				var ajaxCall = 'multicom-pos/public/api/v1/items/func/report?';
				var x = app.searchVar;
				var y = app.orderVar;
