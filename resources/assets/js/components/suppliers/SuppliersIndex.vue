	    ...
	    &nbsp;<br>
	      <table class="table table-bordered table-striped table-dark">
	          ...
	          <tr>
	            ...
	            <th v-on:click="sort('deadline',5)">Deadline &nbsp;<span v-show="showAsc[5]" class="glyphicon glyphicon-chevron-up"></span> <span v-show="showDesc[5]" class="glyphicon glyphicon-chevron-down"></span></th>
			  ...
			  <tr>
				...
				<td>
				  <div class="row">
					<div class="col-xs-6 col-md-12">
					  <div class="input-group">
						<input type="number" class="form-control" placeholder="Search" v-model="searchVar.deadline" v-on:keyup="search()">
					  </div>
					</div>
				  </div>
				</td>
	          ...
	          <tr v-for="supplier, index in suppliers">
	            ...
	            <template v-else>
					<td><input type="text" v-model="supplier.code" class="form-control"><p style="color:red;" v-if="!$v.suppliers.$each[index].code.required">The code field is required!</p></td>
					<td><input type="text" v-model="supplier.name" class="form-control"><p style="color:red;" v-if="!$v.suppliers.$each[index].name.required">The name field is required!</p></td>
					<td><input type="text" v-model="supplier.address" class="form-control"><p style="color:red;" v-if="!$v.suppliers.$each[index].address.required">The address field is required!</p></td>
					<td><input type="text" v-model="supplier.city" class="form-control"><p style="color:red;" v-if="!$v.suppliers.$each[index].city.required">The city field is required!</p></td>
					<td><input type="text" v-model="supplier.phone" class="form-control"><p style="color:red;" v-if="!$v.suppliers.$each[index].phone.required">The phone field is required!</p></td>
					<td><input type="number" v-model="supplier.deadline" class="form-control"><p style="color:red;" v-if="!$v.suppliers.$each[index].deadline.required">The deadline field is required!</p><p v-if="!$v.suppliers.$each[index].deadline.numeric">The deadline field must be numeric!</p></td>
	              	<button
	                 	class="btn btn-xs btn-success"
	                 	v-on:click="updateEntry(supplier.id,index)" type="button" :disabled="$v.suppliers.$invalid">
	                	 Update
	              	</button>
	              	<button
...
<script>
	...
	export default {
        ...
        mounted() {
            var app = this;
            axios.get('multicom-pos/public/api/v1/suppliers?page='+ app.pagination.current_page)
                .then(function (resp) {
        ...
        methods: {
			...
			refresh(){
				...
				var ajaxCall = 'multicom-pos/public/api/v1/suppliers?';
				var x = app.searchVar;
				var y = app.orderVar;
            ...
            updateEntry(id, index){
              var app = this;
              var newSupplier = app.suppliers[index];
              axios.patch('multicom-pos/public/api/v1/suppliers/' + id, newSupplier)
                  .then(function (resp) {
                  	alert(app.suppliers[index].code + " " + app.suppliers[index].name + " was updated");
                  })
                  .catch(function (resp) {
                      console.log(resp);
                      alert("Could not edit your supplier");
                  });
                app.editInput.splice(index, 1, false);
                location.reload();
            },
            deleteEntry(id, index) {
                if (confirm("Do you really want to delete it?")) {
                    var app = this;
                    axios.delete('multicom-pos/public/api/v1/suppliers/' + id)
                        .then(function (resp) {
                        	alert(app.suppliers[index].code + " " + app.suppliers[index].name + " was deleted");
